...
pub const ChannelChunk = struct {
	...
	fn propagateDirect(self: *ChannelChunk, lightQueue: *main.utils.CircularBufferQueue(Entry)) void {
		...
		while(lightQueue.dequeue()) |entry| {
			...
			for(chunk.Neighbors.iterable) |neighbor| {
				...
				if(result.value[0] != 0 or result.value[1] != 0 or result.value[2] != 0) {
					if(self.isSun and result.value[0] == 255 and result.value[1] == 255 and result.value[2] == 255) {
						lightQueue.enqueueFront(result);
					} else {
	...
	pub fn uniformSun(self: *ChannelChunk) void {
		...
		self.data.palette[1] = .{255, 255, 255};
		self.mutex.unlock();
