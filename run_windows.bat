...
if not "%version%" == "%currVersion%" (
	...
	powershell -Command $ProgressPreference = 'SilentlyContinue'; "Invoke-WebRequest -uri https://pkg.machengine.org/zig/%version%.zip -OutFile compiler\archive.zip"
	if errorlevel 1 (
...
echo Building Zig Cubyz Launcher (%*^) from source. This may take a few minutes...
...
echo Cubyz Launcher successfully built!
echo Launching Cubyz Launcher.

zig-out\bin\Cubyz-Launcher
