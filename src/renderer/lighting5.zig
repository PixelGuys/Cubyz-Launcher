...
fn DynamicPackedIntArray(size: comptime_int) type {
	return struct {
		data: []u8 = &.{},
		bitSize: u5 = 0,
		...
		fn initCapacity(allocator: main.utils.NeverFailingAllocator, bitSize: u5) Self {
			return .{
		...
		fn deinit(self: *Self, allocator: main.utils.NeverFailingAllocator) void {
			allocator.free(self.data);
			self.* = .{};
		}

		fn resize(self: *Self, allocator: main.utils.NeverFailingAllocator, newBitSize: u5) void {
		...
		fn getValue(self: *const Self, i: usize) u32 {
		...
		fn setValue(self: *Self, i: usize, value: u32) void {
			std.debug.assert(i < size);
		...
		fn setAndGetValue(self: *Self, i: usize, value: u32) u32 {
			std.debug.assert(i < size);
			if(self.bitSize == 0) return 0;
			const bitIndex = i*self.bitSize;
			const byteIndex = bitIndex >> 3;
...
pub const ChannelChunk = struct {
	data: DynamicPackedIntArray(chunk.chunkVolume),
	palette: [][3]u8,
	...
	fn setValueInternal(self: *ChannelChunk, i: usize, val: [3]u8) void {
		...
		while(paletteIndex < self.paletteLength) : (paletteIndex += 1) {
	...
	fn optimizeLayout(self: *ChannelChunk) void {
		...
		var newData = DynamicPackedIntArray(chunk.chunkVolume).initCapacity(main.globalAllocator, @intCast(std.math.log2_int_ceil(u32, self.activePaletteEntries)));
